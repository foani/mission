...
const ADMIN_KEY = process.env.ADMIN_PRIVATE_KEY || '' // 하드코딩된 관리자 개인키
...
class BlockchainService {
  ...
  constructor() {
    this.provider = new ethers.providers.JsonRpcProvider(CATENA_RPC)
    this.wallet = new ethers.Wallet(ADMIN_KEY, this.provider)
    
    // 스마트컨트랙트 ABI (간략된 버전)
    const ABI = [
      'function executeAirdrop(address recipient, uint256 amount, string rewardType)',
      'function executeBatchAirdrop(address[] recipients, uint256[] amounts, string rewardType)',
      'function getGlobalStats() view returns (uint256, uint256, uint256, uint256)',
      'function getUserAirdropHistory(address user) view returns (tuple[])',
      'event AirdropExecuted(address indexed recipient, uint256 amount, string rewardType, uint256 timestamp)'
    ]
    
    this.contract = new ethers.Contract(AIRDROP_CONTRACT, ABI, this.wallet)
  }
  
  /**
   * 개별 에어드롭 실행
   */
  async executeAirdrop(recipient: string, amount: string, rewardType: string): Promise<AirdropResult> {
    try {
      console.log(`에어드롭 실행: ${recipient}, ${amount} CTA`)
      
      const tx = await this.contract.executeAirdrop(
        recipient,
        ethers.utils.parseEther(amount),
        rewardType
      )
   ...
   * 배치 에어드롭 실행
   */
  async executeBatchAirdrop(recipients: string[], amounts: string[], rewardType: string): Promise<AirdropResult[]> {
    try {
      console.log(`배치 에어드롭: ${recipients.length}명`)
      
      const amountsWei = amounts.map(amount => ethers.utils.parseEther(amount))
      
      const tx = await this.contract.executeBatchAirdrop(recipients, amountsWei, rewardType)
      const receipt = await tx.wait()
      ...
      console.error('배치 에어드롭 실패:', error)
      return recipients.map((recipient, index) => ({
        recipient,
        amount: amounts[index],
        rewardType,
  ...
  async testConnection(): Promise<boolean> {
    try {
      const blockNumber = await this.provider.getBlockNumber()
      console.log(`CreataChain 블록: ${blockNumber}`)
      return true
    } catch (error) {
      console.error('CreataChain 연결 실패:', error)
      return false
    }
  }
}

export { BlockchainService, AirdropResult }
